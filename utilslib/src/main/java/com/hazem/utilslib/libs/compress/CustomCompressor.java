...
class CustomCompressor {
    ...
    private static final String TAG = "Compressor Compress";

    private final CompressBuilder mLuban;

    private ByteArrayOutputStream mByteArrayOutputStream;

    CustomCompressor(CompressBuilder luban) {
        mLuban = luban;
    }
    ...
    Observable<List<File>> multiAction(List<File> files) {
        ...
        return Observable.zip(observables, new Function<Object[], List<File>>() {
            @Override
            public List<File> apply(Object[] args) {
    ...
    private File compressImage(int gear, File file) throws IOException {
        switch (gear) {
            case Compressor.THIRD_GEAR:
                return thirdCompress(file);
            case Compressor.CUSTOM_GEAR:
                return customCompress(file);
            case Compressor.FIRST_GEAR:
